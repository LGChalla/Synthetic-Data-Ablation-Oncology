...
==========================================
Generates records under the full G(x) gate without any retrieval
context. Prompts contain only the schema and the case description.

This is the RAG ablation's baseline — same gate, same model,
same TNM cells, same seeds as run_rag.py. The only difference
is the absence of MedCPT-retrieved PubMed abstracts.

Note: Shares corpus with Ablations 1 and 2 (condition=full_norag).
Checkpoint is reused if already generated.

Trains:  adapter_D_full_norag
Outputs: results/phase1_full_norag.jsonl

Usage:
  python ablations/3_rag_vs_norag/run_norag.py
  python ablations/3_rag_vs_norag/run_norag.py --runs 32
"""
...
from core.logging_utils import make_record, append_jsonl, load_completed, mark_completed, load_jsonl
...
def generate(model_id: str, n_runs: int, results_dir: str) -> list:
    ...
    if len(completed) >= n_runs:
        print(f"  All {n_runs} runs already in checkpoint. Loading existing records.")
    ...
    for i in range(n_runs):
        ...
        prompt  = (
            ...
            "No RAG context provided — generate from clinical knowledge only."
        )

        print(f"  [{i+1:>3}/{n_runs}] {t}/{n}/{m}  rag=False", end="  ", flush=True)
        raw          = generate_one(model, tokenizer, prompt, cfg.GEN_PARAMS,
                                    rag_context="", use_json_stop=True, model_id=model_id)
...
def main():
    ...
    print("\n" + "="*65)
    print("ABLATION 3 — FULL GATE, NO RAG  |  Adapter D")
    ...
    print(f"\nDone. Next: run run_rag.py then compare.py")
