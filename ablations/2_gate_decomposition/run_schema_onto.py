...
Condition: SCHEMA + ONTOLOGY  (Adapter C)
==========================================
Generates synthetic records and admits only those that pass both
JSON schema completeness (C1) AND SNOMED CT ontology coverage (C2).
AJCC logic is computed and logged but does NOT filter the corpus.

Trains:  adapter_C_schema_onto
Outputs: results/phase1_schema_onto.jsonl
         adapters/adapter_C_schema_onto/final_adapter/

Usage:
  python ablations/2_gate_decomposition/run_schema_onto.py
  python ablations/2_gate_decomposition/run_schema_onto.py --runs 64
"""
...
def generate(model_id: str, n_runs: int, results_dir: str) -> list:
    ...
    for i in range(n_runs):
        ...
        admit        = gate_result.schema and gate_result.ontology   # C1 + C2

        print(f"C1={gate_result.schema} C2_onto={gate_result.ontology} "
    ...
    print(f"\n  C1+C2 admitted: {admitted}  |  Rejected: {rejected}")
...
def main():
    parser = argparse.ArgumentParser(description="Ablation 2 — Schema + Ontology (C2)")
    ...
    print("\n" + "="*65)
    print("ABLATION 2 — SCHEMA + ONTOLOGY (C2)  |  Adapter C")
    ...
    if not args.skip_training:
        print("\n[Phase 4] Training adapter_C_schema_onto...")
        train_adapter(CONDITION, args.results_dir, args.adapters_dir)

    print(f"\nDone. Next: run run_full_gate.py then compare.py")
